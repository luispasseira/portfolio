/* tab.js — the browser tab joins the system. The favicon mirrors the
   Orchestra (ember during triage, a green flash on merge), goes full
   alert during a self-debug incident, and the title tells the truth
   when you leave: the suite really does pause. */
(function () {
  const LP = window.LP;
  ...
  function draw() {
    try {
      ...
      if (mode === 'paused') {
        ...
        const col = mode === 'triage' ? COL.ember : COL.signal;
        c.fillStyle = col;
  ...
  /* chaos.js raises and clears the alarm */
  LP.tabAlert = on => {
    alertOn = !!on;
  ...
  /* orchestra drives the everyday mood */
  if (LP.orchestra) {
  ...
  /* the tab tells the truth when you leave */
